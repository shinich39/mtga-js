...
__export(mtga_exports, {
  ...
  AutoIndentModule: () => AutoIndentModule,
...
var onKeydown = function(e) {
  ...
  e.preventDefault();
  const module2 = this.getModule(HistoryModule.name);
  if (!module2) {
...
var singleLineHandler = function(e) {
  ...
  const module2 = this.getModule(CommentModule.name);
  if (!module2) {
...
var onKeydown3 = function(e) {
  ...
  e.preventDefault();
  const module2 = this.getModule(IndentModule.name);
  if (!module2) {
...
var onKeydown4 = function(e) {
  ...
  const module2 = this.getModule(AutoIndentModule.name);
  if (!module2) {
  ...
  e.preventDefault();
  const { pairs, indentUnit } = module2;
  const el = this.element;
  const { short, long, dir, isReversed } = getState(el);
  const prevChar = el.value.charAt(short - 1);
...
var closePairHandler = function(e) {
  ...
  const module2 = this.getModule(AutoPairModule.name);
  if (!module2) {
    console.warn(`Module not found: ${AutoPairModule.name}`);
    return;
  }
  const pairs = module2.pairs;
  const { key, altKey, ctrlKey, shiftKey } = parseKeyboardEvent(e);
...
var onKeyup2 = function(e) {
  ...
  const module2 = this.getModule(AutoCompleteModule.name);
  if (!module2) {
    console.warn(`Module not found: ${AutoCompleteModule.name}`);
    return;
  }
  module2.stop(true);
  const requestId = module2._requestId + 1;
  const chunkSize = module2._chunkSize;
  const result = [];
  ...
  let candidates = [];
  if (text) {
    const index = findIndex(module2.indexes, text);
    if (index) {
      candidates = index.tags;
    } else {
      candidates = module2.tags;
    }
  }
  const processChunk = () => {
    const chunks = [];
    ...
    if (isKilled || module2._requestId !== requestId) {
      return;
    }
